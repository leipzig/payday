...
.hotDates<?echo$uc?> {  font-family: Arial, Helvetica, sans-serif; font-size: <?echo$fsn?>px; font-weight: <?echo$fwn?>; color: #cc9900}
.storedDates<?echo$uc?> {  font-family: Arial, Helvetica, sans-serif; font-size: <?echo$fsn?>px; font-weight: <?echo$fwn?>; color: #074366}
...
$calBiData= new biData;
$calBiData->findbyDay(strtotime($mo."/".max($cd,1)."/".$yr));
$calTimeMatrix = new timeMatrix;
if ($calTimeMatrix->recordExists($calBiData->id,$viewUserid))
{
	echo "<tr ".$al." CLASS=\"storedDates".$uc."\"";
	if($ntc) {echo " BGCOLOR=\"#$ntc\"";}?> onMouseOut="javascript: this.className='storedDates<?PHP echo $uc?>'" onMouseOver="javascript: this.className='hotDates<?PHP echo $uc?>'" onMouseDown="window.location='index.php?id=<?PHP echo $calBiData->id;?>&userid=<?PHP echo $viewUserid;?>'"><?PHP
}
 else
{
	echo "<tr ".$al." CLASS=\"dates".$uc."\"";
	if($ntc) {echo " BGCOLOR=\"#$ntc\"";}?> onMouseOut="javascript: this.className='dates<?PHP echo $uc?>'" onMouseOver="javascript: this.className='hotDates<?PHP echo $uc?>'" onMouseDown="window.location='index.php?id=<?PHP echo $calBiData->id;?>&userid=<?PHP echo $viewUserid;?>'"><?PHP
}?>
 <td><? echo $calBiData->id;//use max to prevent neg numbers when referring to previous month on first row?></td>
<?
